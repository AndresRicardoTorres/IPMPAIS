...
void EstudianteDAO::crearTablas()
{
    ...
    objPg->query("CREATE TABLE estudiante"
		 "( año integer"
		 ...
		 ", tipo_colegio character varying"
		 ...
		 ", competencia2 real"
		 ", competencia3 real"
		 ", CONSTRAINT estudiante_pkey PRIMARY KEY (codigo))");
    objPg->query("DROP TABLE IF EXISTS estudiante_tipo;");
    objPg->query("CREATE TABLE estudiante_tipo"
		 "( año integer"
		 ", codigo character varying"
		 ", CONSTRAINT estudiante_tipo_pkey PRIMARY KEY (codigo ))");
    objPg->query("DROP TABLE IF EXISTS estudiante_creditos;");
    objPg->query("CREATE TABLE estudiante_creditos"
		 "( año integer"
		 ", codigo character varying"
		 ", CONSTRAINT estudiante_creditos_pkey "
                 " PRIMARY KEY (codigo))");
    delete objPg;
}

const char* EstudianteDAO::insertar( encabezadoCSV encabezados
				   , datosCSV datosIn
				   , wxGauge *barraProgreso){
    int columna_codigo       = -1
      , columna_documento    = -1
      , columna_estrato      = -1
      , columna_comuna       = -1
      , columna_barrio       = -1
    ...
    ///<!-- Coloco limite al indicador en la GUI -->
    barraProgreso->SetRange(datosIn.size());

    ///<!-- Busco en los nombres de columnas correspondientes al texto -->
    ...
    i=0;
    for ( std::list<std::list<std::string> >::iterator it = datosIn.begin()
	; it != datosIn.end()
	; it++,i++
	){
        ...
        sql = "INSERT INTO estudiante"
	      ...
	      ", tipo_colegio"
	      ") VALUES "
	      "(" + ano + ",'" + codigo + "','" +  documento + "','" + estrato
	      + "','" + comuna + "','" + barrio + "','" + ciudad + "','"
...
const char* EstudianteDAO::insertarICFES( encabezadoCSV encabezados
                                        , datosCSV datosIn
                                        , wxGauge *barraProgreso
                                        )
{
...
void EstudianteDAO::crearColumnasAsignaturas(std::string codigo_asignatura)
{
    ...
    sql = "ALTER TABLE estudiante_tipo DROP COLUMN \"TIPO_"
        + codigo_asignatura + "\";";
    objPg->query(sql.c_str());
    sql = "ALTER TABLE estudiante_creditos DROP COLUMN \"CREDITOS_"
        + codigo_asignatura  + "\";";
    objPg->query(sql.c_str());
    sql = "ALTER TABLE estudiante ADD COLUMN \"" + codigo_asignatura
        + "\" character varying;";
    objPg->query(sql.c_str());
    sql = "ALTER TABLE estudiante_tipo ADD COLUMN \"TIPO_" + codigo_asignatura
        + "\" character varying;";
    objPg->query(sql.c_str());
    sql = "ALTER TABLE estudiante_creditos ADD COLUMN \"CREDITOS_"
...
bool EstudianteDAO::actualizar(Calificacion* unaCalificacion)
{
    ...
    // Actualizo la tabla estudiante_tipo con la forma como curso el estudiante
    // la asignatura

    sql1 << "UPDATE estudiante_tipo SET \"TIPO_"
         << unaCalificacion->getCodigoAsignatura()
         << "\" = "<<unaCalificacion->getTipoNumericoCursado()
         << " WHERE codigo = '" << unaCalificacion->getCodigoEstudiante()
         << "';";
    afectadas = objPg->update(sql1.str().c_str());
    if(0==afectadas){
        sql2 << "INSERT INTO estudiante_tipo (año,codigo,\"TIPO_"
             << unaCalificacion->getCodigoAsignatura() << "\") VALUES ("
             << unaCalificacion->getAnno() << ",'"
             << unaCalificacion->getCodigoEstudiante() << "',"
             << unaCalificacion->getTipoNumericoCursado() << ");";
    ...
    // Actualizo la tabla estudiante creditos con los creditos que tenia la
    // asignatura cuando la curso

    sql1 << "UPDATE estudiante_creditos SET \"CREDITOS_"
         << unaCalificacion->getCodigoAsignatura() << "\" = '"
         << unaCalificacion->getCreditos() << "' WHERE codigo = '"
         << unaCalificacion->getCodigoEstudiante() << "';";

    afectadas = objPg->update(sql1.str().c_str());
    if(0==afectadas){
        sql2 << "INSERT INTO estudiante_creditos (año,codigo,\"CREDITOS_"
             << unaCalificacion->getCodigoAsignatura() << "\") VALUES ("
             << unaCalificacion->getAnno() << ",'"
             << unaCalificacion->getCodigoEstudiante() << "','"
             << unaCalificacion->getCreditos() << "');";
    ...
    sql1 << "UPDATE estudiante SET \""
         << unaCalificacion->getCodigoAsignatura() << "\" = '"
         << unaCalificacion->getCalificacion() << "' WHERE codigo = '"
         << unaCalificacion->getCodigoEstudiante() << "';";
    afectadas = objPg->update(sql1.str().c_str());

    if(0 == afectadas){
        sql2 << "INSERT INTO estudiante (año,codigo,\""
             << unaCalificacion->getCodigoAsignatura() << "\") VALUES ("
             << unaCalificacion->getAnno() << ",'"
             << unaCalificacion->getCodigoEstudiante() << "','"
             << unaCalificacion->getCalificacion() << "');";
         afectadas = objPg->insert(sql2.str().c_str());
        if(0 != afectadas){
            exito = true;
        }
...
std::string EstudianteDAO::insertarPuntajesECAES( encabezadoCSV encabezados
                                                , datosCSV datosIn
                                                , wxGauge *barraProgreso
                                                )
{
    ...
    int columna_documento     = -1
      , columna_puntaje       = -1
      , columna_componente_1  = -1
      , columna_componente_2  = -1
      , columna_componente_3  = -1
      , columna_componente_4  = -1
      , columna_componente_5  = -1
      , columna_componente_6  = -1
      , columna_componente_7  = -1
      , columna_competencia_1 = -1
      , columna_competencia_2 = -1
      , columna_competencia_3 = -1
      ;
    ...
    int i = 0
      , j = 0
      ;
    for ( listaCSV::iterator it = encabezados.begin()
        ; it != encabezados.end()
        ; it++,i++
        ){
        std::string stringIt =  *it;
        stringIt = limpiarString(stringIt);
        found = stringIt.find("documento");
        if (found != std::string::npos)
          columna_documento = i;
        found = stringIt.find("documentodeidentidad");
        if (found != std::string::npos)
          columna_documento = i;
        found = stringIt.find("identificacion");
        if (found != std::string::npos)
          columna_documento = i;
        found = stringIt.find("puntajetotal");
        if (found != std::string::npos)
          columna_puntaje = i;
        found = stringIt.find("puntaje");
        if (found != std::string::npos)
          columna_puntaje = i;
        found = stringIt.find("componente1p");
        if (found!=std::string::npos)
          columna_componente_1 = i;
        found = stringIt.find("componente2p");
        if (found!=std::string::npos)
          columna_componente_2 = i;
        found = stringIt.find("componente3p");
        if (found != std::string::npos)
          columna_componente_3 = i;
        found=stringIt.find("componente4p");
        if (found != std::string::npos)
          columna_componente_4 = i;
        found = stringIt.find("componente5p");
        if (found != std::string::npos)
          columna_componente_5 = i;
        found = stringIt.find("componente6p");
        if (found != std::string::npos)
          columna_componente_6 = i;
        found = stringIt.find("componente7p");
        if (found != std::string::npos)
          columna_componente_7 = i;
        found = stringIt.find("competencias1p");
        if (found != std::string::npos)
          columna_competencia_1 = i;
        found = stringIt.find("competencias2p");
        if (found != std::string::npos)
          columna_competencia_2 = i;
        found = stringIt.find("competencias3p");
        if (found != std::string::npos)
          columna_competencia_3 = i;
    }

    std::string documento,puntaje
              , componente_1
              , componente_2
              , componente_3
              , componente_4
    ...
    for ( datosCSV::iterator it = datosIn.begin()
        ...
        if(0==documento.size() && 0==puntaje.size())
          continue;
        ...
        sstm << "UPDATE estudiante SET ecaes_total = "<< puntaje
             << ",componente1 =" << componente_1
             << ",componente2 =" << componente_2
             << ",componente3 =" << componente_3
             << ",componente4 =" << componente_4
             << ",componente5 =" << componente_5
             << ",componente6 =" <<componente_6
             << ",componente7 =" <<componente_7
             << ",competencia1 =" <<competencia_1
             << ",competencia2 =" <<competencia_2
             << ",competencia3 =" <<competencia_3
...
listaCSV* EstudianteDAO::getListaEstudiantesOrdenadaPorPromedio
  ( int fecha_inicio
  , int fecha_final
...
{
    ResultadoConsulta *resultado = selectAll( "codigo"
                                            , fecha_inicio
                                            , fecha_final
                                            , filtro_texto
                                            , ECAESoRegistro
...
puntajesICFES * EstudianteDAO::getPuntajesICFES( int fecha_inicio
                                               , int fecha_final
                                               )
{
    // El orden de los campos de la consulta NO se debe cambiar, corresponde al
    // orden en admisionesunivalle
    return selectAll("codigo"
                    ",len"
                    ",mat"
                    ",CASE WHEN cis IS NULL THEN (geo+his)/2"
                    " ELSE cis END as cis"
                    ",fil"
                    ",bio"
                    ",qui"
...
ResultadoConsulta* EstudianteDAO::selectAll( std::string columnas
                                           , int fecha_inicio
                                           , int fecha_final
    ...
    if(ECAESoRegistro){
      ...
      if(listadoAsignaturas.size() != 11){
            std::cout << "ERROR listadoAsignaturas.size()"
                      << listadoAsignaturas.size() << std::endl;
        ...
        ss << "SELECT " << columnas
           << " FROM estudiante JOIN (SELECT codigo_estudiante, promedio FROM"
              " (SELECT count(codigo_asignatura) as cantidad"
              " , codigo_estudiante"
              " , SUM(calificacion_numerica*creditos) / SUM(creditos)"
              " as promedio FROM calificacion WHERE creditos!= 0 ";

        if(listadoAsignaturas.size()>0)
            ss<<" AND codigo_asignatura IN("<<listadoAsignaturas<<")";

        ss << " GROUP BY codigo_estudiante ) as a ";
        ss << ") as subconsulta ON (codigo_estudiante = codigo) WHERE"
              " len is not null AND"
              " mat is not null AND"
              " fil IS NOT NULL AND"
              " bio IS NOT NULL AND"
              " qui IS NOT NULL AND"
              " fis IS NOT NULL AND"
              " (cis IS NOT NULL OR (his IS NOT NULL AND geo IS NOT NULL))";
    }
...
std::string EstudianteDAO::obtenerNombreColumnas(){
    PG *objPg = new PG(conexion.c_str());
    std::stringstream ss;
    ResultadoConsulta *resultado = objPg->select("SELECT column_name "
